    ...
    public function searchData(Request $request)
    {
        ...
        return Datatables::of($objects)
			...
			->editColumn('price', function ($object) {
				return $object->price ? formatCurrent($object->price) : 'Liên hệ';
			})
            ...
            ->editColumn('manufacturer_id', function ($object) {
                return $object->manufacturer ? $object->manufacturer->name : '';
            })
...
//                $result = $result . ' <a href="" title="Thêm vào danh mục đặc biệt" class="dropdown-item add-category-special"><i class="fa fa-angle-right"></i>Thêm vào danh mục đặc biệt</a>';
			...
			->addIndexColumn()
			->rawColumns(['action', 'category_special'])
			->make(true);
    }
	...
	public function store(ProductStoreRequest $request)
	{
		...
		try {
			...
			$object->solution_id = $request->solution_id;
            ...
            // thuộc tính
            $attributesRequest = $request->input('attrs');

            if($attributesRequest) {
                foreach ($attributesRequest as $attribute) {
                    if(@$attribute['values']) {
                        foreach ($attribute['values'] as $values) {
                            $attValue = new AttributeValue();
                            $attValue->product_id = $object->id;
	...
	public function delete($id)
    {
		...
		if (!$object->canDelete()) {
            ...
            ProductCategorySpecial::query()->where('product_id', $id)->delete();
            AttributeValue::query()->where('product_id', $object->id)->delete();
