...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        $categories = Category::query()
            ...
            ->with(['products' => function ($q) {
                $q->with('image')->where('status', 1)->latest();
            }])
            ->get();
        $categories->each(function ($cat) {
            $cat->setRelation('products', $cat->products->take(10));
        });

        $data['cateParents'] = Category::query()->with('image')
            ->where('parent_id', 0)->where('show_home_page', 1)->get();

        $data['categories'] = $categories;
        $data['manufactures'] = Manufacturer::query()->with('image')->latest()->get();
        $data['posts'] = Post::query()->with('image')->latest()->limit(4)->get();
    ...
    public function getCategoryProduct($slug) {
        ...
        return view($this->view . '.product_list', $data);
    }

    public function getProductDetail(Request $request, $slug) {
        $product = Product::query()->with(['category', 'image', 'galleries.image', 'manufacturer', 'attrs'])->where('slug', $slug)->firstOrFail();
        $attributes = $product->attrs->toArray();
        ...
        $policies = Policy::query()->where('status', 1)->get();

        return view($this->view . '.product_detail', compact('product', 'otherProducts', 'policies'));
    }

    public function getManufactureProduct(Request $request, $slug) {
        $sortMap = [
            'price_asc'  => ['price', 'asc'],
        ...
        $manufacture = Manufacturer::findBySlug($slug);
        $products = Product::query()->where('manufacturer_id', $manufacture->id)
            ->orderBy($sortColumn, $sortDirection)
            ->paginate(16)->appends($request->only('sort'));
        $cateIds = Product::query()->where('manufacturer_id', $manufacture->id)
            ->where('status', 1)
            ->pluck('cate_id')->toArray();
        $categories = Category::query()->whereIn('id', $cateIds)->get();

        return view($this->view . '.product_manufacture', compact('products', 'manufacture', 'categories'));
    }

    public function getProductByManuAndCate(Request $request, $cateSlug, $manufactureSlug) {
        $sortMap = [
            'price_asc'  => ['price', 'asc'],
            'price_desc' => ['price', 'desc'],
        ];
        ...
        $manufacture = Manufacturer::findBySlug($manufactureSlug);
        $category = Category::findBySlug($cateSlug);

        $products = Product::query()
            ->where('manufacturer_id', $manufacture->id)
        ...
        return view($this->view . '.product_manufacture', compact('products', 'manufacture', 'categories', 'category'));
    }
    ...
    public function blogs(Request $request, $slug = null) {
        if ($slug) {
            ...
            $blogs = Post::query()->with(['image', 'category'])
                ->where('status', 1)
        ...
        $othersPost = Post::query()->with(['image', 'category'])
            ->where('status', 1)->latest()->take(3)->get();

        return view($this->view . '.blog', compact('blogs', 'categoryBlog', 'categories', 'othersPost'));
    ...
    public function blogDetail(Request $request, $slug) {
        ...
        $blogLienquan = Post::query()->with('image')
            ->whereNotIn('id', [$blog->id])
        ...
        return view($this->view . '.blog_detail', compact('blog', 'otherBlogs', 'categoryBlog', 'categories', 'blogLienquan'));
    }


    public function policy(Request $request, $slug) {
        $policy = Policy::findBySlug($slug);
        $policies = Policy::query()->where('status', 1)->get();

        return view($this->view . '.policy', compact('policy', 'policies'));
    }

    public function about_page(Request $request) {
        $about = AboutPage::query()->with(['image', 'image_back', 'image_banner'])->find(1);
        $policies = Policy::query()->where('status', 1)->get();

        return view($this->view . '.about_page', compact('about', 'policies'));
    }
    ...
    public function searchProducts(Request $request) {
        ...
        $products = $products->orderBy($sortColumn, $sortDirection)
            ->paginate(20) ->appends(array_merge(
                $request->only('sort'),
                ['keywords' => $keyword]
            ));

        return view($this->view . '.search', compact('products', 'keyword'));
    }
    ...
    public function contact(Request $request) {
        ...
        return view($this->view . '.contact_us', compact('config'));
    }
    ...
    public function getServiceDetail(Request $request, $slug) {
        $service = Service::query()->with(['category', 'image', 'image_back'])->where('slug', $slug)->firstOrFail();
    ...
    public function getChildCategory(Request $request, $slug) {
        $category = Category::query()->with(['childs.image', 'banner'])
            ->where('slug', $slug)->firstOrFail();

        return view('site.product_category', compact('category'));
    }

    public function getListProduct(Request $request, $slug) {
        $category = Category::query()->with(['image', 'banner'])
            ->where('slug', $slug)->firstOrFail();
        $cateParent = Category::query()->with(['childs', 'banner'])->where('id', $category->parent_id)->first();

        $products = Product::query()->with('image')
            ->where(['cate_id' => $category->id, 'status' => 1])->latest()->paginate(15);

        return view('site.product_list', compact('category', 'products', 'cateParent'));
    }
